...
export default function AdminRefundsPage({
  ...
  const updateReturnRequest = (request, nextStatus) => {
    router.patch(`/admin/return-requests/${request.id}`, {
      status: nextStatus,
      admin_note: nextStatus === 'approved'
        ? 'Approved for support follow-up.'
        : nextStatus === 'rejected'
  ...
  return (
    <AdminLayout title="Refunds">
      ...
      <AdminCard title={`Return & Exchange Requests (${returnRequests.length})`} className="mt-6">
        <div className="overflow-hidden rounded-2xl border border-slate-200">
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs font-black uppercase tracking-wide text-slate-500">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                ...
                {returnRequests.map((request) => (
                  <tr key={request.id} className="transition hover:bg-slate-50">
                    <td className="px-5 py-4">
                      <Link href={`/admin/orders/${request.order_id}`} className="font-black text-primary hover:underline">
                        #{request.order_id}
                      </Link>
                      <p className="mt-1 text-xs font-semibold text-slate-500">{new Date(request.created_at).toLocaleDateString()}</p>
                    </td>
                    <td className="px-5 py-4 font-semibold text-slate-700">
                      {request.order?.customer_name || request.order?.user?.name || 'Customer'}
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-black text-slate-900">{request.item?.product_name || '-'}</p>
                      <p className="text-xs font-semibold text-slate-500">Qty {request.item?.quantity || 1}</p>
                    </td>
                    <td className="max-w-xs px-5 py-4">
                      <p className="font-black capitalize text-slate-900">{request.type}</p>
                      <p className="text-xs font-semibold text-slate-600">{request.reason}</p>
                    ...
                    <td className="px-5 py-4 text-xs font-bold text-slate-600">
                      <p>Current: {[request.current_size, request.current_color].filter(Boolean).join(' / ') || '-'}</p>
                      <p className="mt-1 text-rose-600">Requested: {[request.requested_size, request.requested_color].filter(Boolean).join(' / ') || '-'}</p>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        {request.status === 'pending' && (
                          <>
                            <button type="button" onClick={() => updateReturnRequest(request, 'approved')} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">
                              Approve
                            </button>
                            <button type="button" onClick={() => updateReturnRequest(request, 'rejected')} className="rounded-xl bg-red-50 px-3 py-2 text-xs font-black text-red-700">
                              Reject
                            </button>
                          </>
                        )}
                        {request.status === 'approved' && (
                          <button type="button" onClick={() => updateReturnRequest(request, 'completed')} className="rounded-xl bg-primary px-3 py-2 text-xs font-black text-white">
                            Complete
                          </button>
                        )}
                      </div>
